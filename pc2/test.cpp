...
bool showAxes = true;
bool show3DRepresentation = false; // Nueva variable para mostrar la representación 3D
...
void createMenus();
void draw3DRepresentation(); // Nuevo prototipo para la representación 3D
...
// Función para calcular y dibujar la curva B-spline
void drawBSplineCurve() {
...
// Función para dibujar la representación 3D de los puntos ingresados
void draw3DRepresentation() {
    ...
    glBegin(GL_LINE_STRIP);
    for (auto& point : controlPoints) {
        glVertex3f(point.first, point.second, 0.0f); // Z = 0 para una línea plana en XY
    }
    glEnd();
}
...
void drawRightSection() {
    ...
    // Aquí puedes agregar más objetos 3D si lo deseas
}
...
void mouseClick(int button, int state, int x, int y) {
    if (x > windowWidth / 2) return; // Solo en la sección izquierda
