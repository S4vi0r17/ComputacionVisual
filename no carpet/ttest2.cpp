...
#include <stdio.h>
#include <math.h>
#include <GL/glut.h>  // Ajuste la ruta del include si es necesario.
...
GLdouble angulo = 0;
GLint modelo = 1; // Iniciar con la circunferencia como modelo por defecto.
...
void menu_color(int opcion) {
    ...
    glutPostRedisplay(); // Actualiza la pantalla después de cambiar el color.
}

void menu_modelo(int opcion) {
    modelo = opcion;
    glutPostRedisplay(); // Actualiza la pantalla después de cambiar el modelo.
}
